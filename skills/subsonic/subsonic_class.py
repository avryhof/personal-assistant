...
import settings
from personal_assistant.model_helpers import md5
from skills.subsonic.exceptions import InvalidCriteria
...
class Subsonic(object):
    ...
    def __init__(self, **kwargs):
        ...
        if self.password:
            ...
            self.salt = "".join(random.choice(string.ascii_letters + "1234567890_") for i in range(salt_length))

            salted_password = self.password + self.salt
            self.encoded_password = md5(salted_password)

    def _call_url(self, resource, data={}):
        api_data = dict(
            u=self.username, t=self.encoded_password, s=self.salt, v=self.api_version, c=self.app_name, f="json"
        )
        ...
        url_params = urlencode(api_data)

        url = "{}/rest/{}.view?{}".format(self.endpoint, resource, url_params)
    ...
    def download_url(self, song_id=False):
        if not song_id:
            song = self.now_playing()
            song_id = song.get("id")
        return self._call_url("download", dict(id=song_id))
    ...
    def stream_url(self, song_id=False):
        if not song_id:
            song = self.now_playing()
            song_id = song.get("id")
        return self._call_url("stream", dict(id=song_id))
